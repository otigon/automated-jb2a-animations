<script>
    ...
    $: color = menuType === "" || animation === "" || variant === "" ? "" : color;
    ...
    async function switchChange() {
        ...
        if (newSwitchType === "on") {
            ...
            color = Object.keys( aaColorMenu.range[menuType][animation][variant])[0];
        }
    }
    async function menuTypeChange() {
        ...
        variant = Object.keys(
            aaVariantMenu.range[newMenuType][animation]
        )[0];
        color = Object.keys(
    ...
    async function animationChange() {
        ...
        variant = Object.keys(
            aaVariantMenu.range[menuType][newAnimation]
        )[0];
        color = Object.keys(
    ...
    $: isDisabled = (switchType === "custom" && returningWeapons.includes(animation) && menuType === "weapon") || switchType === "on" ? false : true
...
<div class="aa-select-animation" transition:fade={{duration:500}}>
    <div class="flexcol" style="grid-row: 1 / 2;grid-column: 2 / 3;">
        <label for="1">{localize("AUTOANIM.ranged")} {localize("AUTOANIM.switch")}</label>
        <select
    ...
    <div class="flexcol {isDisabled ? "opacityButton opacityLabel" : ""}" style='grid-row:1/2; grid-column:3/4'>
        <label for="">Set Return</label>
        <button disabled={isDisabled} class="oldCheck {returnEnabled && !isDisabled ? "selected" : "notSelected"}" on:click={() => switchLabel()}>{returnLabel}</button>
    </div>
...
<div class="aa-4wide" transition:fade={{duration: 500}}>
    <div class="flexcol"  style='grid-row:1/2; grid-column:2/3'>
        ...
        <button class="oldCheck" on:click={() => switchDetect()}>{detectLabel}</button>
    </div>
    <div class="flexcol {detect === "auto" ? "opacityButton" : ""}" style="grid-row: 1 / 2; grid-column: 3 / 5;">
        <label for="">{localize("AUTOANIM.gridSquares")}</label>
        <input disabled={detect === "auto"} type="Number" bind:value={range} placeholder=2>
    </div>
</div>
{/if}
